...
time_series_total <- scaled_global_data %>% 
  ggplot(aes(x=Month,y=total_incidents))+geom_point(color="darkolivegreen4")+geom_path(color="darkolivegreen") + theme_classic() + labs(x="Month",y="Total Incidents [t]") + theme(axis.text.x = element_text(angle = 60, hjust=1),axis.title.x = element_blank()) + scale_x_date(date_breaks = "3 months", date_labels = "%Y-%m")
